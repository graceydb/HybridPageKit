...
@interface RNSComponentViewPool ()
...
@implementation RNSComponentViewPool

+ (RNSComponentViewPool *)shareInstance {
    static dispatch_once_t once;
    static RNSComponentViewPool *componentPool = nil;
    dispatch_once(&once,^{
        componentPool = [[RNSComponentViewPool alloc] init];
    });
            ...
            for (__kindof UIView *view in viewSetTmp) {
                if(view.superview == superView || !view.superview){
                    ...
                    if([view isMemberOfClass:[UIView class]]){
                        //有状态的view复用，placeholderView remove 所有的subviews
